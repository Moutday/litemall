    ...
    methods: {
        ...
        onSelect() {
            ...
            const success = (res) => {
              ...
              this.tempFilePaths = tempFilePath.map((item) => ({ url: item, uid: this.getUid(), thumbTempFilePath: res.thumbTempFilePath }))
        ...
        onRemove(e) {
            ...
            const prodId = [];
            fileList.map((item) => {
                var video = JSON.parse(item.res.data);
                prodId.push(video.id)
            })
